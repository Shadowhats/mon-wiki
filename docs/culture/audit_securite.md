...
| Commande | Action |
| :--- | :--- |
| `nmap -sP 192.168.1.0/24` | **Ping Sweep** : Liste les machines allumées sur le réseau. |
| `nmap -sV -sC -T4 192.168.1.50` | **Scan Standard** : Détermine les versions des services (`-sV`) et lance les scripts de détection par défaut (`-sC`). |
| `nmap -p- 192.168.1.50` | **Scan Full** : Analyse les 65535 ports TCP (très lent mais exhaustif). |
| `nmap --script vuln 192.168.1.50` | **Scan de vulnérabilités** : Utilise la base de scripts Nmap pour trouver des failles connues (CVE). |

### Rustscan - Le challenger ultra-rapide
Écrit en Rust, il est capable de scanner l'intégralité des ports d'une machine en quelques secondes avant de passer le relais à Nmap pour l'analyse.
...
* **Sortie :** Un fichier HTML interactif avec des recommandations précises (ex: "Liaisons SMB non signées", "Comptes administrateurs dormants").
...
* **Objectif :** Visualiser comment un simple compte "Utilisateur" peut devenir "Admin du Domaine" en 3 sauts (ex: Session ouverte sur un serveur mal sécurisé).
...
* **Exemple (Vérifier qui a les droits admin sur le réseau) :** `nxc smb 192.168.1.0/24 -u 'p.tech' -p 'P@ssword123!' --local-auth`
* **Exemple (Lister les partages réseau anonymes) :** `nxc smb 192.168.1.0/24 --shares`
...
| Logiciel | Type | Usage |
| :--- | :--- | :--- |
| **OpenVAS (Greenbone)** | 🟢 Open Source | Scanner complet à installer sur un serveur Debian. Génère des rapports PDF très détaillés. |
| **Tenable Nessus** | 🔴 Pro | La référence pro. Très précis, moins de faux positifs, mais licence payante (version Essentials gratuite pour 16 IP). |
| **Qualys** | 🔴 Cloud | Utilisé pour scanner des infrastructures externes ou des parcs multi-sites via le cloud. |

---

## 🕸️ 4. Audit Web (Sites et Applications)

Identifier les dossiers cachés ou les failles de type injection.
...
### Nikto
Scanner web "old school" mais efficace pour trouver des fichiers de configuration oubliés ou des serveurs mal patchés.
...
* **John the Ripper :** Le plus polyvalent.
  * `john --wordlist=passwords.txt my_hashes.txt`
* **Hashcat :** Le plus puissant (utilise la puissance de la carte graphique GPU).
  * `hashcat -m 1000 hashes.txt rockyou.txt` (Le `-m 1000` correspond au format NTLM de Windows).
...
| **Metasploit Framework** | Une base de données d'exploits. Une fois la faille trouvée (avec Nmap ou Nessus), Metasploit permet de tester si l'exploitation est réellement possible. |
| **Impacket** | Une collection de scripts Python pour manipuler les protocoles réseau (indispensable pour l'audit AD avancé). |
...
1. **Scan de ports :** `nmap -sV [IP]` (Vérifier qu'aucun service inutile ne tourne).
