...
> **💡 Astuce :** Pensez au parcours d'un mail. Il est envoyé au serveur (SMTP), puis l'utilisateur vient le récupérer (POP/IMAP).
...
| Port | Protocole | Type | Description |
| :--- | :--- | :--- | :--- |
| **53** | **DNS** | UDP/TCP | *Domain Name System*. Traduit les noms (`google.com`) en adresses IP. UDP pour les requêtes, TCP pour les transferts de zones. |
| **67 / 68** | **DHCP** | UDP | Distribue les adresses IP (67 = Serveur, 68 = Client). |
| **123** | **NTP** | UDP | *Network Time Protocol*. Synchronisation de l'heure des serveurs (critique pour l'Active Directory). |
| **161 / 162**| **SNMP** | UDP | Supervision réseau (Zabbix, PRTG). Le 161 pour interroger, le 162 pour recevoir des alertes (Traps). |
| **389** | **LDAP** | TCP/UDP | Annuaire (Active Directory). Utilisé pour l'authentification. |
...
* **Tester si un port est ouvert sur un serveur distant :**
  `Test-NetConnection -ComputerName 192.168.1.50 -Port 3389` (Remplace le vieux Telnet pour faire des tests).
...
* **Tester un port spécifique rapidement :**
  `nc -zv 192.168.1.50 3306` (Utilise Netcat pour tester la connexion MySQL).